...
MyApplet.prototype = {
    ...
    _init: function (metadata, orientation, panel_height) {
		...
		try {
			...
			let netGraph = new Graph(this.graphArea, netProvider, this._panelHeight);
            ...
            // Request space for three graphs where w=h*ScaleRatio each
	...
	_update: function() {
		...
		Mainloop.timeout_add(REFRESH_TIMEOUT, Lang.bind(this, this._update));
	},
    ...
    onGraphRepaint: function (area) {
        try {
            ...
            for (let index = 0; index < this.graphs.length; index++) {
				if(index != 0)
				area.get_context().translate((this._panelHeight * ScaleRatio), 0);
...
Graph.prototype = {
    ...
    paint: function (area, panel_height) {
        ...
        // Label
		cr.setFontSize(9*global.ui_scale);
...
NetDataProvider.prototype = {
	...
	_init: function(){
		...
		for (let i = 0; i < dev.length; ++i)
			this.devices.push(dev[0].get_iface());
		
		this.down_last = this.getNetLoad();
	},
	...
	getNetLoad: function() {
		...
		{
			try{
		    	GTop.glibtop_get_netload(this.gtop, this.devices[i]);
			} catch(e) {}
	...
	getData: function()
	{
		let down = this.getNetLoad();
		if(this.max < (down - this.down_last))
		{
			this.max = (down - this.down_last);
		}
		let down_delta = (down - this.down_last) / this.max;
		this.down_last = down;
		return down_delta;
	},
	...
	getName: function()
	{
		return "NET: " + Math.round(this.max / 1024) + "kb/s";
	}
};
